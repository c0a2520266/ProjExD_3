...
def main():
    ...
    while True:
            ...
            for j, beam in enumerate(beams):
                if beam is not None and bomb is not None:
                    if beam.rct.colliderect(bomb.rct):
                        explosions.append(Explosion(bomb.rct)) # 爆発生成 
                        bombs[i] = None
                        beams[j] = None
                        score.score += 1 # スコア加算 
        ...
        for exp in explosions: exp.update(screen)
        score.update(screen)
